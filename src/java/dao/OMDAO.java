...
public class OMDAO {
    ...
    public void inserir(OM om) {
        String sql = "INSERT INTO ordens (numero_ordem, id_fornecedor, placa, modelo, data, status, descricao) VALUES (?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = Conexao.getConnection()) {
            ...
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                ...
                stmt.setInt(2, om.getIdFornecedor());
                stmt.setString(3, om.getPlaca());
                stmt.setString(4, om.getModelo());
                stmt.setDate(5, om.getData());
                stmt.setString(6, om.getStatus());
                stmt.setString(7, om.getDescricao());
                stmt.executeUpdate();
            }
    ...
    String sql = "UPDATE ordens SET id_fornecedor=?, placa=?, modelo=?, data=?, status=?, descricao=? WHERE id=?";
    try (Connection conn = Conexao.getConnection();
         PreparedStatement stmt = conn.prepareStatement(sql)) {

        stmt.setInt(1, om.getIdFornecedor());
